...
void LaunchTMax(T *out, T *src0, T *src1, void *stream)
{
    if constexpr ( std::is_same_v<T, aclFloat16> )
        runTMAX<half, kGRows_, kGCols_, kTRows_, kTCols_, kVRows_, kVCols_, padValueType><<<1, nullptr, stream>>>((half*)(out),
                                                                                  (half*)(src0),
                                                                                  (half*)(src1));
    else 
        runTMAX<T, kGRows_, kGCols_, kTRows_, kTCols_, kVRows_, kVCols_, padValueType><<<1, nullptr, stream>>>(out, src0, src1);
}

template void LaunchTMax<float, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(float *out, float *src0, float *src1, void *stream);
template void LaunchTMax<int32_t, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(int32_t *out, int32_t *src0, int32_t *src1, void *stream);
template void LaunchTMax<aclFloat16, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void LaunchTMax<int16_t, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(int16_t *out, int16_t *src0, int16_t *src1, void *stream);

template void LaunchTMax<float, 60, 60, 64, 64, 60, 60, PAD_VALUE_MAX>(float *out, float *src0, float *src1, void *stream);
template void LaunchTMax<int32_t, 60, 60, 64, 64, 60, 60, PAD_VALUE_MAX>(int32_t *out, int32_t *src0, int32_t *src1, void *stream);
template void LaunchTMax<aclFloat16, 1, 3600, 2, 4096, 1, 3600, PAD_VALUE_MAX>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void LaunchTMax<int16_t, 16, 200, 20, 512, 16, 200, PAD_VALUE_MAX>(int16_t *out, int16_t *src0, int16_t *src1, void *stream);
