...
void LaunchTMin(T *out, T *src0, T *src1, void *stream) {
    if constexpr (std::is_same_v<T, aclFloat16>)
        runTMIN<half, dstTileH, dstTileW, src0TileH, src0TileW, src1TileH, src1TileW, vRows, vCols, padValueType>
            <<<1, nullptr, stream>>>((half *)(out), (half *)(src0), (half *)(src1));
    else
        runTMIN<T, dstTileH, dstTileW, src0TileH, src0TileW, src1TileH, src1TileW, vRows, vCols, padValueType>
            <<<1, nullptr, stream>>>(out, src0, src1);
}

template void LaunchTMin<float, 64, 64, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(
    float *out, float *src0, float *src1, void *stream);
template void LaunchTMin<int32_t, 64, 64, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(
    int32_t *out, int32_t *src0, int32_t *src1, void *stream);
template void LaunchTMin<aclFloat16, 64, 64, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(
    aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void LaunchTMin<int16_t, 64, 64, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(
    int16_t *out, int16_t *src0, int16_t *src1, void *stream);

template void LaunchTMin<float, 60, 128, 64, 64, 60, 128, 60, 60, PAD_VALUE_MIN>(
    float *out, float *src0, float *src1, void *stream);
template void LaunchTMin<int32_t, 60, 128, 64, 64, 60, 128, 60, 60, PAD_VALUE_MIN>(
    int32_t *out, int32_t *src0, int32_t *src1, void *stream);
template void LaunchTMin<aclFloat16, 1, 3600, 2, 4096, 1, 3600, 1, 3600, PAD_VALUE_MIN>(
    aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void LaunchTMin<int16_t, 16, 256, 20, 512, 16, 256, 16, 200, PAD_VALUE_MIN>(
    int16_t *out, int16_t *src0, int16_t *src1, void *stream);