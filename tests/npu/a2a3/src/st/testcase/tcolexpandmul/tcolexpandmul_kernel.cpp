...
void launchTColExpandMul(T *out, T*src0, T*src1, void *stream) {
    if constexpr (std::is_same_v<T, aclFloat16>) {
        runCOLEXPANDMUL<half, dstRow, dstCol, src1Row, src1Col><<<1, nullptr, stream>>>((half*)out, (half*)src0, (half*)src1);
    } else {
...
template void launchTColExpandMul<aclFloat16, 4, 256, 1, 256>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void launchTColExpandMul<aclFloat16, 10, 64, 1, 64>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
}