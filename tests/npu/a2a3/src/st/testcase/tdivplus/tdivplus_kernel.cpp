...
__global__ AICORE void runTDiv(
...
void LaunchTDiv(T *out, T *src0, T *src1, void *stream) {
    if constexpr (std::is_same_v<T, aclFloat16>)
        runTDiv<half, dstTileH, dstTileW, src0TileH, src0TileW, src1TileH, src1TileW>
            <<<1, nullptr, stream>>>((half *)(out), (half *)(src0), (half *)(src1), vRows, vCols);
    else
        runTDiv<T, dstTileH, dstTileW, src0TileH, src0TileW, src1TileH, src1TileW>
            <<<1, nullptr, stream>>>(out, src0, src1, vRows, vCols);
}

template void LaunchTDiv<aclFloat16, 16, 64, 16, 128, 16, 128, 16, 64>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void LaunchTDiv<float, 16, 32, 16, 64, 16, 32, 16, 32>(float *out, float *src0, float *src1, void *stream);
template void LaunchTDiv<aclFloat16, 16, 64, 16, 128, 16, 128, 16, 63>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void LaunchTDiv<float, 16, 32, 16, 64, 16, 32, 16, 31>(float *out, float *src0, float *src1, void *stream);