...
bool TPartMaxTest()
{
    ...
    LaunchTPartMax<T, kGRowsD_, kGColsD_, kGRowsS0_, kGColsS0_, kGRowsS1_, kGColsS1_,
...
TEST_F(TPARTMAXTest, test0) {
    bool res = TPartMaxTest<float, 16, 32, 16, 16, 16, 32,
        16, 32, 16, 16, 16, 32>();
    EXPECT_TRUE(res);
}
TEST_F(TPARTMAXTest, test1) {
    bool res = TPartMaxTest<float, 22, 32, 22, 32, 16, 32,
        22, 32, 22, 32, 16, 32>();
    EXPECT_TRUE(res);
}
TEST_F(TPARTMAXTest, test2) {
    bool res = TPartMaxTest<float, 22, 40, 22, 40, 22, 32,
    22, 40, 22, 40, 22, 32>();
    EXPECT_TRUE(res);
}
TEST_F(TPARTMAXTest, test3) {
    bool res = TPartMaxTest<float, 22, 40, 22, 40, 8, 40,
        22, 40, 22, 40, 8, 40>();
    EXPECT_TRUE(res);
}
TEST_F(TPARTMAXTest, test4) {
    bool res = TPartMaxTest<float, 64, 128, 64, 128, 64, 128,
        64, 128, 64, 128, 64, 128>();
    EXPECT_TRUE(res);
}
TEST_F(TPARTMAXTest, testEmpty0) {
    bool res = TPartMaxTest<float, 16, 32, 16, 0, 16, 32,
        16, 32, 16, 8, 16, 32>();
    EXPECT_TRUE(res);
}
TEST_F(TPARTMAXTest, testEmpty1) {
    bool res = TPartMaxTest<float, 16, 32, 0, 32, 16, 32,
        16, 32, 8, 32, 16, 32>();
    EXPECT_TRUE(res);
}
TEST_F(TPARTMAXTest, testEmpty2) {
    bool res = TPartMaxTest<float, 16, 32, 16, 32, 16, 0,
        16, 32, 16, 32, 16, 8>();
    EXPECT_TRUE(res);
}
TEST_F(TPARTMAXTest, testEmpty3) {
    bool res = TPartMaxTest<float, 16, 32, 16, 32, 0, 32,
        16, 32, 16, 32, 8, 32>();
    EXPECT_TRUE(res);
}
