...
void LaunchTCmp(uint8_t *out, T *src0, T *src1, void *stream)
{
    if constexpr (std::is_same_v<T, aclFloat16> )
        runTCmp<half, kGRows_, kGCols_, kTRows_, kTCols_, cmpMode><<<1, nullptr, stream>>>((out),
                                                                                  (half*)(src0),
                                                                                  (half*)(src1));
    else 
...
template void LaunchTCmp<int32_t, 64, 64, 32, 32, CmpMode::EQ>(uint8_t *out, int32_t *src0, int32_t *src1, void *stream);
template void LaunchTCmp<int32_t, 16, 32, 16, 32, CmpMode::EQ>(uint8_t *out, int32_t *src0, int32_t *src1, void *stream);
template void LaunchTCmp<float, 128, 128, 64, 64, CmpMode::LE>(uint8_t *out, float *src0, float *src1, void *stream);
template void LaunchTCmp<int32_t, 77, 81, 32, 32, CmpMode::EQ>(uint8_t *out, int32_t *src0, int32_t *src1, void *stream);
template void LaunchTCmp<int32_t, 32, 32, 32, 32, CmpMode::EQ>(uint8_t *out, int32_t *src0, int32_t *src1, void *stream);