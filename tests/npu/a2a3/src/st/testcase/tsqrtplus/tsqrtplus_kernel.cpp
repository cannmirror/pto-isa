...
void LaunchTSqrt(T *out, T *src, void *stream)
{
    if constexpr (std::is_same_v<T, aclFloat16>)
        runTSqrt<half, dstTileRow, dstTileCol, srcTileRow, srcTileCol, validRow, validCol, isInPlace>
            <<<1, nullptr, stream>>>((half *)(out), (half *)(src));
    else
        runTSqrt<T, dstTileRow, dstTileCol, srcTileRow, srcTileCol, validRow, validCol, isInPlace>
            <<<1, nullptr, stream>>>(out, src);
}

template void LaunchTSqrt<float, 64, 64, 64, 128, 64, 64, true>(float *out, float *src, void *stream);
template void LaunchTSqrt<float, 128, 64, 64, 64, 64, 64, false>(float *out, float *src, void *stream);
template void LaunchTSqrt<aclFloat16, 64, 64, 128, 128, 64, 64, true>(aclFloat16 *out, aclFloat16 *src, void *stream);
template void LaunchTSqrt<aclFloat16, 64, 256, 64, 64, 64, 64, false>(aclFloat16 *out, aclFloat16 *src, void *stream);