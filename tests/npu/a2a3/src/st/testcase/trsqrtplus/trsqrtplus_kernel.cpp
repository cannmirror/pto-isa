...
void LaunchTRsqrt(T *out, T *src, void *stream)
{
    if constexpr (std::is_same_v<T, aclFloat16>)
        runTRsqrt<half, dstTileRow, dstTileCol, srcTileRow, srcTileCol, validRow, validCol, isInPlace>
            <<<1, nullptr, stream>>>((half *)(out), (half *)(src));
    else
        runTRsqrt<T, dstTileRow, dstTileCol, srcTileRow, srcTileCol, validRow, validCol, isInPlace>
            <<<1, nullptr, stream>>>(out, src);
}

template void LaunchTRsqrt<float, 64, 64, 64, 128, 64, 64, true>(float *out, float *src, void *stream);
template void LaunchTRsqrt<float, 128, 64, 64, 64, 64, 64, false>(float *out, float *src, void *stream);
template void LaunchTRsqrt<aclFloat16, 64, 64, 128, 128, 64, 64, true>(aclFloat16 *out, aclFloat16 *src, void *stream);
template void LaunchTRsqrt<aclFloat16, 64, 256, 64, 64, 64, 64, false>(aclFloat16 *out, aclFloat16 *src, void *stream);