...
void LaunchTDiv(T *out, T *src0, T *src1, void *stream)
{
    if constexpr (sameTile) {
        ...
        runTDIV<T, dstTileH, dstTileW, src0TileH, src0TileW, src1TileH,
...
{
    if constexpr (sameTile) {
        runTDIV<half, dstTileH, dstTileW, vRows, vCols><<<1, nullptr, stream>>>
            ((half*)(out), (half*)(src0), (half*)(src1));
    } else {
        runTDIV<half, dstTileH, dstTileW, src0TileH, src0TileW, src1TileH,
            src1TileW, vRows, vCols><<<1, nullptr, stream>>>
            ((half*)(out), (half*)(src0), (half*)(src1));
    }
}

template void LaunchTDiv<float, 64, 64, 64, 64, 64, 64, 64, 64, true>
    (float *out, float *src0, float *src1, void *stream);
template void LaunchTDiv<int32_t, 64, 64, 64, 64, 64, 64, 64, 64, true>
    (int32_t *out, int32_t *src0, int32_t *src1, void *stream);
template void LaunchTDiv<int16_t, 64, 64, 64, 64, 64, 64, 64, 64, true>
    (int16_t *out, int16_t *src0, int16_t *src1, void *stream);
template void LaunchTDivHalf<16, 256, 16, 256, 16, 256, 16, 256, true>
