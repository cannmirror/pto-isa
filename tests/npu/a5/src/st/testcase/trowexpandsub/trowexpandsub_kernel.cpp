...
__global__ AICORE void runROWEXPANDSUB(__gm__ T __out__ *out, __gm__ T __in__ *src0, __gm__ T __in__ *src1) {
    ...
    TROWEXPANDSUB<DstTileData, TileData>(dstTile, src0Tile, src1Tile);
...
void launchTRowExpandSub(T *out, T*src0, T*src1, void *stream) {
    if constexpr (std::is_same_v<T, aclFloat16>) {
        runROWEXPANDSUB<half, dstRow, dstCol, src1Row, src1Col><<<1, nullptr, stream>>>((half*)out, (half*)src0, (half*)src1);
    } else {
        runROWEXPANDSUB<T, dstRow, dstCol, src1Row, src1Col><<<1, nullptr, stream>>>(out, src0, src1);
    }
}

template void launchTRowExpandSub<float, 8, 128, 8, 1>(float *out, float *src0, float *src1, void *stream);
template void launchTRowExpandSub<float, 18, 32, 18, 1>(float *out, float *src0, float *src1, void *stream);
template void launchTRowExpandSub<aclFloat16, 10, 256, 10, 1>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void launchTRowExpandSub<aclFloat16, 12, 64, 12, 1>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
}