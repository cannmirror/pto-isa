...
void launchTRowExpandExpdif(T *out, T*src0, T*src1, void *stream) {
    if constexpr (std::is_same_v<T, aclFloat16>) {
        runROWEXPANDEXPDIF<half, dstRow, dstCol, src1Row, src1Col><<<1, nullptr, stream>>>((half*)out, (half*)src0, (half*)src1);
    } else {
        runROWEXPANDEXPDIF<T, dstRow, dstCol, src1Row, src1Col><<<1, nullptr, stream>>>(out, src0, src1);
    }
}

template void launchTRowExpandExpdif<float, 32, 64, 32, 1>(float *out, float *src0, float *src1, void *stream);
template void launchTRowExpandExpdif<float, 16, 32, 16, 1>(float *out, float *src0, float *src1, void *stream);
template void launchTRowExpandExpdif<aclFloat16, 16, 32, 16, 1>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void launchTRowExpandExpdif<aclFloat16, 48, 64, 48, 1>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
}