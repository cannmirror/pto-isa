...
void launchTRowExpandAdd(T *out, T*src0, T*src1, void *stream) {
    if constexpr (std::is_same_v<T, aclFloat16>) {
        runROWEXPANDADD<half, dstRow, dstCol, src1Row, src1Col, src0eqdst><<<1, nullptr, stream>>>((half*)out, (half*)src0, (half*)src1);
    } else {
...
void launchTRowExpandAdd2(T *out, T*src0, T*src1, void *stream) {
    if constexpr (std::is_same_v<T, aclFloat16>) {
        runROWEXPANDADD2<half, dstRow, dstCol, src1Row, src1Col, src0eqdst><<<1, nullptr, stream>>>((half*)out, (half*)src0, (half*)src1);
    } else {
...
template void launchTRowExpandAdd<aclFloat16, 48, 64, 48, 1, true>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void launchTRowExpandAdd<aclFloat16, 16, 128, 16, 1, true>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
template void launchTRowExpandAdd2<float, 24, 64, 24, 8, true>(float *out, float *src0, float *src1, void *stream);
template void launchTRowExpandAdd<aclFloat16, 32, 64, 32, 1, false>(aclFloat16 *out, aclFloat16 *src0, aclFloat16 *src1, void *stream);
