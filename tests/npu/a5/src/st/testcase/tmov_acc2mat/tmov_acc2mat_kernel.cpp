...
template <typename OutType, typename AType, typename BType, int validM, int validK, int validN, int row, int col,
    bool isRelu = false, Layout layoutType = Layout::ND, int sfractalSize = 512>
__global__ AICORE void RunTMOV(__gm__ OutType *out, __gm__ AType *src0, __gm__ BType *src1)
{
    ...
    uint8_t syncId = 0;
    using SrcTileData = Tile<TileType::Mat, OutType, row, col, GetTileBLayout<layoutType>(), validM, validN,
...
template <typename OutType, typename AType, typename BType, typename FbType, int validM, int validK, int validN,
    int row, int col, bool isRelu = false, Layout layoutType = Layout::ND, int sfractalSize = 512>
...
void LaunchTMOVAcc2MatNZ2NZ(uint8_t *out, uint8_t *src0, uint8_t *src1, void *stream)
{
    if constexpr (tilingKey == 1) {
        RunTMOV<half, half, half, 96, 80, 112, 96, 112, false, Layout::NZ><<<1, nullptr, stream>>>(
...
{
    if constexpr (tilingKey == 1) {
        RunTMOV<half, half, half, 65, 40, 80, 80, 80, true><<<1, nullptr, stream>>>(
            reinterpret_cast<half *>(out), reinterpret_cast<half *>(src0), reinterpret_cast<half *>(src1));
    } else if constexpr (tilingKey == 2) {
...
{
    if constexpr (tilingKey == 1) {
        RunTMOV<half, half, half, 80, 40, 66, 80, 80, false, Layout::DN><<<1, nullptr, stream>>>(
            reinterpret_cast<half *>(out), reinterpret_cast<half *>(src0), reinterpret_cast<half *>(src1));
    } else if constexpr (tilingKey == 2) {
        RunTMOV<float, half, half, 88, 48, 95, 96, 96, false, Layout::DN><<<1, nullptr, stream>>>(
            reinterpret_cast<float *>(out), reinterpret_cast<half *>(src0), reinterpret_cast<half *>(src1));
    } else if constexpr (tilingKey == 3) {
...
{
    if constexpr (tilingKey == 1) {
        RunTMOVFBQuant<half, half, half, uint64_t, 128, 64, 64, 128, 64, false, Layout::NZ>
        ...
        RunTMOVFBQuant<int8_t, half, half, uint64_t, 128, 64, 64, 128, 64, false, Layout::NZ>
            <<<1, nullptr, stream>>>(reinterpret_cast<int8_t *>(out), reinterpret_cast<half *>(src0),
                reinterpret_cast<half *>(src1), reinterpret_cast<uint64_t *>(src2));
    } else if constexpr (tilingKey == 3) {
        RunTMOVFBQuant<half, int8_t, int8_t, uint64_t, 128, 128, 64, 128, 64, true, Layout::NZ>
        ...
        RunTMOVFBQuant<int8_t, int8_t, int8_t, uint64_t, 64, 128, 128, 64, 128, true, Layout::NZ>
...
{
    if constexpr (tilingKey == 1) {
        RunTMOVFBQuant<half, half, half, uint64_t, 111, 47, 96, 112, 96, true>
            <<<1, nullptr, stream>>>(reinterpret_cast<half *>(out), reinterpret_cast<half *>(src0),
                reinterpret_cast<half *>(src1), reinterpret_cast<uint64_t *>(src2));
    } else if constexpr (tilingKey == 2) {
        RunTMOVFBQuant<int8_t, half, half, uint64_t, 60, 128, 64, 64, 64, true>
            <<<1, nullptr, stream>>>(reinterpret_cast<int8_t *>(out), reinterpret_cast<half *>(src0),
                reinterpret_cast<half *>(src1), reinterpret_cast<uint64_t *>(src2));
    } else if constexpr (tilingKey == 3) {
...
{
    if constexpr (tilingKey == 1) {
        RunTMOVFBQuant<half, half, half, uint64_t, 80, 80, 80, 80, 80, false, Layout::DN>
            <<<1, nullptr, stream>>>(reinterpret_cast<half *>(out), reinterpret_cast<half *>(src0),
                reinterpret_cast<half *>(src1), reinterpret_cast<uint64_t *>(src2));
    } else if constexpr (tilingKey == 2) {
        RunTMOVFBQuant<int8_t, half, half, uint64_t, 96, 128, 60, 96, 64, false, Layout::DN>
            <<<1, nullptr, stream>>>(reinterpret_cast<int8_t *>(out), reinterpret_cast<half *>(src0),
                reinterpret_cast<half *>(src1), reinterpret_cast<uint64_t *>(src2));
    } else if constexpr (tilingKey == 3) {
        RunTMOVFBQuant<half, int8_t, int8_t, uint64_t, 64, 48, 60, 64, 64, true, Layout::DN>
            <<<1, nullptr, stream>>>(reinterpret_cast<half *>(out), reinterpret_cast<int8_t *>(src0),
                reinterpret_cast<int8_t *>(src1), reinterpret_cast<uint64_t *>(src2));
    } else if constexpr (tilingKey == 4) {
        RunTMOVFBQuant<int8_t, int8_t, int8_t, uint64_t, 64, 64, 90, 64, 96, true, Layout::DN>
            <<<1, nullptr, stream>>>(reinterpret_cast<int8_t *>(out), reinterpret_cast<int8_t *>(src0),
                reinterpret_cast<int8_t *>(src1), reinterpret_cast<uint64_t *>(src2));
    }
}
...
{
    if constexpr (tilingKey == 1) {
        RunTMOVSCQuant<half, half, half, 112, 48, 96, 112, 96, true, Layout::NZ><<<1, nullptr, stream>>>(
            reinterpret_cast<half *>(out), reinterpret_cast<half *>(src0), reinterpret_cast<half *>(src1), 4);
    } else if constexpr (tilingKey == 2) {
        RunTMOVSCQuant<int8_t, half, half, 112, 96, 64, 112, 128, true, Layout::NZ><<<1, nullptr, stream>>>(
            reinterpret_cast<int8_t *>(out), reinterpret_cast<half *>(src0), reinterpret_cast<half *>(src1), 3);
    } else if constexpr (tilingKey == 3) {
        RunTMOVSCQuant<half, int8_t, int8_t, 32, 128, 64, 32, 64, false, Layout::NZ><<<1, nullptr, stream>>>(
            reinterpret_cast<half *>(out), reinterpret_cast<int8_t *>(src0), reinterpret_cast<int8_t *>(src1), 5);
    } else if constexpr (tilingKey == 4) {
        RunTMOVSCQuant<int8_t, int8_t, int8_t, 64, 32, 64, 64, 64, false, Layout::NZ><<<1, nullptr, stream>>>(
...
void LaunchTMOVAcc2MatSCQuantNZ2ND(uint8_t *out, uint8_t *src0, uint8_t *src1, void *stream)
{
    if constexpr (tilingKey == 1) {
        ...
        RunTMOVSCQuant<half, int8_t, int8_t, 30, 48, 64, 32, 64, true><<<1, nullptr, stream>>>(
            reinterpret_cast<half *>(out), reinterpret_cast<int8_t *>(src0), reinterpret_cast<int8_t *>(src1), 5);
    } else if constexpr (tilingKey == 4) {
        RunTMOVSCQuant<int8_t, int8_t, int8_t, 60, 48, 32, 64, 32, true><<<1, nullptr, stream>>>(
            reinterpret_cast<int8_t *>(out), reinterpret_cast<int8_t *>(src0), reinterpret_cast<int8_t *>(src1), 2);
    }
}
...
{
    if constexpr (tilingKey == 1) {
        RunTMOVSCQuant<half, half, half, 80, 40, 66, 80, 80, true, Layout::DN><<<1, nullptr, stream>>>(
            reinterpret_cast<half *>(out), reinterpret_cast<half *>(src0), reinterpret_cast<half *>(src1), 4);
    } else if constexpr (tilingKey == 2) {
        RunTMOVSCQuant<int8_t, half, half, 96, 128, 60, 96, 64, true, Layout::DN><<<1, nullptr, stream>>>(
            reinterpret_cast<int8_t *>(out), reinterpret_cast<half *>(src0), reinterpret_cast<half *>(src1), 3);
    } else if constexpr (tilingKey == 3) {
        RunTMOVSCQuant<half, int8_t, int8_t, 128, 128, 64, 128, 64, false, Layout::DN><<<1, nullptr, stream>>>(
            reinterpret_cast<half *>(out), reinterpret_cast<int8_t *>(src0), reinterpret_cast<int8_t *>(src1), 5);
    } else if constexpr (tilingKey == 4) {
        RunTMOVSCQuant<int8_t, int8_t, int8_t, 64, 64, 90, 64, 96, false, Layout::DN><<<1, nullptr, stream>>>(
            reinterpret_cast<int8_t *>(out), reinterpret_cast<int8_t *>(src0), reinterpret_cast<int8_t *>(src1), 2);
    }
}
