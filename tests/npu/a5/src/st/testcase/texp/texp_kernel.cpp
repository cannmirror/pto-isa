...
__global__ __aicore__ void runTExp( __gm__ T __out__ *out, __gm__ T __in__ *src) {
...
void LaunchTExp(T *out, T *src, void *stream)
{
    if constexpr ( std::is_same_v<T, aclFloat16> )
        runTExp<half, kGRows_, kGCols_, kTRows_, kTCols_><<<1, nullptr, stream>>>((half*)(out),
                                                                                  (half*)(src));
    else 
        runTExp<T, kGRows_, kGCols_, kTRows_, kTCols_><<<1, nullptr, stream>>>(out, src);
}
