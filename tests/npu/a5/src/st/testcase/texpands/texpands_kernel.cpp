...
__global__ __aicore__ void runTEXPANDS( __gm__ T __out__ *out, float scalar) {
...
void LaunchTExpandS(void *out, float scalar, void *stream)
{
    if constexpr ( std::is_same_v<T, aclFloat16> )
        runTEXPANDS<half, kGRows_, kGCols_, kTRows_, kTCols_, kVRows_, kVCols_, padValueType><<<1, nullptr, stream>>>((half *) out, scalar);
    else 
        runTEXPANDS<T, kGRows_, kGCols_, kTRows_, kTCols_, kVRows_, kVCols_, padValueType><<<1, nullptr, stream>>>((T *) out, scalar);
}
