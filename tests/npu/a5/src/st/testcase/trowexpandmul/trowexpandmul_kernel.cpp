...
__global__ AICORE void runROWEXPANDMUL(__gm__ T __out__ *out, __gm__ T __in__ *src0, __gm__ T __in__ *src1) {
    ...
    TROWEXPANDMUL<DstTileData, TileData>(dstTile, src0Tile, src1Tile);
...
void launchTRowExpandMul(T *out, T*src0, T*src1, void *stream) {
    if constexpr (std::is_same_v<T, aclFloat16>) {
        runROWEXPANDMUL<half, dstRow, dstCol, src1Row, src1Col><<<1, nullptr, stream>>>((half*)out, (half*)src0, (half*)src1);
    } else {
        runROWEXPANDMUL<T, dstRow, dstCol, src1Row, src1Col><<<1, nullptr, stream>>>(out, src0, src1);
    }
}
