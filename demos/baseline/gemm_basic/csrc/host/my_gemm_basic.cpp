...
at::Tensor run_gemm_basic_custom(const at::Tensor &a, const at::Tensor &b_dn) {
    ...
    TORCH_CHECK(a.dim() == 2 && b_dn.dim() == 2, "a and b_dn must be 2D tensors");
    TORCH_CHECK(a.size(0) == 512 && a.size(1) == 2048, "a shape must be [512, 2048]");
    TORCH_CHECK(b_dn.size(0) == 1536 && b_dn.size(1) == 2048, "b_dn shape must be [1536, 2048] (b.t().contiguous())");
