...
## 说明
**比较掩码（Tile Compare Scalar）**

- 功能：Tile 与标量比较掩码

对 `src0` 与标量 `src1` 逐元素比较，比较方式由 `CmpMode` 指定，结果写入 `dst`（掩码 Tile，具体表示由实现定义）。

---

## 汇编语法
```asm
TCMPS.cmpMode %Src, %R , -> %Dst
```
...
| src0 | 输入 Tile 0 |
| scalar | 标量立即数/寄存器值 |
| cmpMode | 比较模式（EQ/NE/LT/GT/GE/LE） |
...
- 编译期约束：
  ...
  - TileType of dst tiles must be TileType::Vec.
...
- 运行期约束：
  - Number of columns of src and dst must be the same.
  - Number of rows of src and dst must be the same.

### 实现检查（A5）
- 支持数据类型（编译期检查）：int32_t, float, uint32_t, int16_t, half, uint16_t, int8_t, uint8_t
