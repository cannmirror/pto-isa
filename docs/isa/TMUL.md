...
$$ \mathrm{dst}_{i,j} = \mathrm{src0}_{i,j} \times \mathrm{src1}_{i,j} $$
...
### 实现检查（A2A3）
- 支持数据类型（编译期检查）：int32_t, int, int16_t, half, float16_t, float, float32_t
- 编译期约束：
...
### 实现检查（A5）
- 支持数据类型（编译期检查）：int32_t, uint32_t, float, int16_t, uint16_t, half, bfloat16_t, uint8_t, int8_t
- 编译期约束：
  - TMUL: Invalid data type.
  - TMUL: not supported Layout type
