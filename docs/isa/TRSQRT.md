...
$$ \mathrm{dst}_{i,j} = 1 / \sqrt{\mathrm{src}_{i,j}} $$
...
- 编译期约束：
  ...
  - TRSQRT: Number of valid columns must not be greater than number of tile columns.
  - TRSQRT: Number of valid rows must not be greater than number of tile rows.
- 运行期约束：
...
### 实现检查（A5）
- 编译期约束：
  - TRSQRT: not supported Layout type
  - TRSQRT: not supported Layout type
