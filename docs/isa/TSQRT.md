...
- 编译期约束：
  ...
  - TSQRT: TileType of src and dst tiles must be TileType::Vec.
  - TSQRT: Number of valid columns must not be greater than number of tile columns.
  - TSQRT: Number of valid rows must not be greater than number of tile rows.
- 运行期约束：
...
### 实现检查（A5）
- 编译期约束：
  - TSQRT: not supported Layout type
  - TSQRT: not supported Layout type
