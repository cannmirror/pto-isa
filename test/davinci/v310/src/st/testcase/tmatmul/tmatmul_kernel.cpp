...
template <typename T, typename U, typename S, typename B, int M, int K, int N, int ValidM, int ValidK, int Valid N, bool is_bias>
...
__aicore__ inline void runTMATMUL_SPLIT_K(__gm__ T *out, __gm__ U *src0, __gm__ S *src1, __gm__ B *src2)
{
    ...
    using TileMatAData = Tile<Location::Mat, U, BASEM, K, BLayout::RowMajor, M, K, SLayout::ColMajor, 512>; // 大N小Z
    using TileMatBData = Tile<Location::Mat, S, K, BASEN, BLayout::ColMajor, K, N, SLayout::RowMajor, 512>; // 大Z小N
