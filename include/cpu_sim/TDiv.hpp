...
namespace pto{
    ...
    void TDiv_Impl(typename tile_shape::TileDType dst,
                            typename tile_shape::TileDType src0,
                            typename tile_shape::TileDType src1,
                            unsigned validRow, unsigned validCol
                        ) {
        for (unsigned int i = 0; i < validRow; ++i) {
            for (unsigned int j = 0; j < validCol; ++j) {
    ...
    __aicore__ PTO_INLINE void TDIV_IMPL(tile_shape &dst, tile_shape &src0, tile_shape &src1) {
