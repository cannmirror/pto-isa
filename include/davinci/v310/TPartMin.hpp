...
__aicore__ PTO_INLINE void TPARTMIN_IMPL(DstTileData &dst, Src0TileData& src0, Src1TileData& src1)
{
    ...
    static_assert (std::is_same_v<T, S0> && std::is_same_v<T, S1>, "TPARTMIN: Input and output types should match" );
    ...
    if (condSrc0EqDst && condSrc1EqDst) { // src0 == src1 == dst
        TBinOper<TPartMinOp<typename DstTileData::DType>, DstTileData, elementsPerRepeat>(dst.data(), src0.data(), src1.data(), dstValidRow, dstValidCol);
    } else if (condDstgeSrc){             // src0 <= dst && src1 <= dst
        TCopyPadOp<TPartMinOp<typename DstTileData::DType>, DstTileData, elementsPerRepeat, Src0RowStride, Src1RowStride, DstRowStride>
