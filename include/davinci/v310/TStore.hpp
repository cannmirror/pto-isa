...
namespace pto
{
    ...
    __aicore__ PTO_INLINE constexpr QuantMode_t GetScalarPreQuantModeGm()
    {
        QuantMode_t quantPre = QuantMode_t::NoQuant;
        if constexpr (std::is_same<SrcType, float>::value) {
            if constexpr ((std::is_same<DstType, __gm__ int8_t>::value) ||
                (std::is_same<DstType, __gm__ uint8_t>::value)) {
    ...
    __aicore__ PTO_INLINE void TStoreAccND(typename GlobalData::DType *dstGlobalAddr,
        __cc__ typename TileData::DType *srcTileAddr, int gShape3, int gShape4, int gStride2, int gStride3,
    ...
    template <typename GlobalData, typename TileData, QuantMode_t quantPre = QuantMode_t::NoQuant>
    __aicore__ PTO_INLINE void TStoreAccNZ(typename GlobalData::DType *dstAddr,
        __cc__ typename TileData::DType *srcAddr, typename GlobalData::DType *dstGlobalAddr,
        __cc__ typename TileData::DType *srcTileAddr, int gShape0, int gShape1, int gShape2, int gShape3, int gShape4,
        int gStride0, int validRow, int validCol)
    {
        uint16_t mSize = validRow;
        uint16_t nSize = validCol;
        constexpr uint16_t srcStride = TileData::Rows;
        ...
        int gStride0, int gStride1, int gStride2, int gStride3, int gStride4, int validRow, int validCol)
    {
        ...
        if constexpr (GlobalData::layout == pto::Layout::NZ) {
            ...
            TStoreAccND<GlobalData, TileData, quantPre>(dstAddr, srcAddr, gShape3, gShape4, gStride2, gStride3,
                validRow, validCol);
        }
    }
    ...
    __aicore__ void TSTORE_IMPL(GlobalData &dst, TileData &src) {
        ...
        if constexpr (TileData::Loc == pto::Location::Vec) {
            ...
            TStore<GlobalData, TileData>(dst.data(), src.data(),
                dst.GetShape(0),
                dst.GetShape(1),
                dst.GetShape(2),
                dst.GetShape(3),
            ...
            TStoreAcc<GlobalData, TileData, quantPre>(dst.data(), src.data(),
                dst.GetShape(0),
                dst.GetShape(1),
                dst.GetShape(2),
                dst.GetShape(3),
    ...
    __aicore__ void TSTORE_IMPL(GlobalData &dst, TileData &src, FpTileData &fp) {
        ...
        TStoreAccFp<GlobalData, TileData, FpTileData, quantPre>(dst.data(), src.data(), fp.data(),
            dst.GetShape(0),
            dst.GetShape(1),
            dst.GetShape(2),
            dst.GetShape(3),
