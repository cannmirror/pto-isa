...
namespace pto {
    ...
    __tf__ __aicore__ void TMul(typename TileData::TileDType __out__ dst,
            ...
            for (int i = 0; i < validRow; i++) {
                if (numLoop) {
                    for (int j = 0; j < numLoop; j++) {
                        vmul(dstPtr + i * stride + j * elementsPerRepeat * REPEAT_MAX,
                ...
                if (remainAfterLoop) {
                    vmul(dstPtr + i * stride + numLoop * elementsPerRepeat * REPEAT_MAX,
    ...
    __aicore__ void TMUL_IMPL(TileData &dst, TileData &src0, TileData &src1) {
        ...
        TMul<TileData, elementsPerRepeat, blockSizeElem, stride>(dst.data(), src0.data(), src1.data(),
                                                                 validRow, validCol);
    }
}
#endif