...
__tf__
PTO_INTERNAL
void TRowExpandSub(typename TileDataDst::TileDType __out__ dst, typename TileDataDst::TileDType __in__ src0,
    typename TileDataSrc1::TileDType __in__ src1, unsigned validRow, unsigned validCol)
{
    ...
    TRowExpandBinaryInstr<RowExpandSubOp<T>, T, U, rowStride>(dstPtr, src0Ptr, src1Ptr, tmpPtr, tmpPtr_, validRow, validCol);
}

template <typename TileDataDst, typename TileDataSrc1>
PTO_INTERNAL
void TROWEXPANDSUB_IMPL(TileDataDst &dst, TileDataDst &src0, TileDataSrc1 &src1) {
    ...
    PTO_ASSERT(src1.GetValidRow() == 1 && src1.GetValidCol() == validRow, "TROWEXPANDSUB: invalid src1 shape.");
    TRowExpandSub<TileDataDst, TileDataSrc1, rowStride>(dst.data(), src0.data(), src1.data(), validRow, validCol);
}
}  // namespace pto
#endif