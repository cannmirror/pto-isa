...
__tf__ PTO_INTERNAL void TSub(typename TileDataDst::TileDType __out__ dstData,
    ...
    if constexpr (std::is_same_v<TileDataDst, TileDataSrc0> && std::is_same_v<TileDataDst, TileDataSrc1>) {
        ...
        BinaryInstr<SubOp<T>, T, TileDataDst, elementsPerRepeat, blockSizeElem, rowStride>(dst, src0, src1, validRow, validCol);
    } else {
        BinaryPlusInstr<SubOp<T>, T, TileDataDst, TileDataSrc0, TileDataSrc1>(dst, src0, src1, validRow, validCol);
    }
}
...
PTO_INTERNAL void TSUB_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
    ...
    static_assert(std::is_same<T, typename TileDataSrc0::DType>::value ||
                  ...
                  "TSUB: The data type of dst must be consistent with of src0 and src1.");
    ...
    static_assert(std::is_same<T, int32_t>::value || std::is_same<T, int>::value ||
                  ...
                  std::is_same<T, float32_t>::value, "TSUB: Invalid data type.");
    static_assert(TileDataDst::isRowMajor && TileDataSrc0::isRowMajor && TileDataSrc1::isRowMajor,
        "TSUB: not supported Layout type.");
    ...
    if ((validRow == src0.GetValidRow() && validCol == src0.GetValidCol()) &&
        ...
        TSub<T, TileDataDst, TileDataSrc0, TileDataSrc1>(dst.data(), src0.data(), src1.data(), validRow, validCol);
    } else {
        PTO_ASSERT(false, "TSUB: dstTile validRow/validCol must be consistent with of src0 and src1.");
    }
}
}  // namespace pto
#endif