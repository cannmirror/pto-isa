                              ...
                              typename TileData::TileDType __in__ src1, unsigned validRows, unsigned validCols)
{
    ...
    if constexpr (dstRowStride == src0RowStride && dstRowStride == src1RowStride) {
        BinaryInstr<SubOp<T>, TileData, elementsPerRepeat, blockSizeElem, dstRowStride>(dstPtr, src0Ptr, src1Ptr,
                                                                                        validRows, validCols);
    } else {
        BinaryInstr<SubOp<T>, TileData, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride, src1RowStride>(
...
PTO_INTERNAL void TSUB_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1)
{
    ...
    if constexpr (std::is_same_v<TileDataDst, TileDataSrc0> && std::is_same_v<TileDataDst, TileDataSrc1>) {
        ...
        TSub<TileDataDst, elementsPerRepeat, blockSizeElem, dstRowStride>(dst.data(), src0.data(), src1.data(),
