...
__tf__ PTO_INTERNAL void TMin(typename TileDataDst::TileDType __out__ dstData,
...
PTO_INTERNAL void TMIN_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
    ...
    static_assert(std::is_same<T, int32_t>::value || std::is_same<T, int>::value ||
                  ...
                  std::is_same<T, float32_t>::value, "TMIN: Invalid data type.");
    static_assert(TileDataDst::isRowMajor && TileDataSrc0::isRowMajor && TileDataSrc1::isRowMajor,
        "TMIN: not supported Layout type.");
    ...
    if ((validRow == src0.GetValidRow() && validCol == src0.GetValidCol()) &&
        ...
        TMin<T, TileDataDst, TileDataSrc0, TileDataSrc1>(dst.data(), src0.data(), src1.data(), validRow, validCol);
    } else {
        TPARTMIN_IMPL(dst, src0, src1);
    }
}
