...
template <typename T, unsigned blockElemSize, unsigned dstStride, unsigned srcStride>
__PTO_INSTR__ void TransposeB32(__ubuf__ T *dstPtr, __ubuf__ T *srcPtr, unsigned num_subtile_x, unsigned num_subtile_y)
{
    ...
    for (unsigned repeat = 0; repeat < num_repeat;
         ++repeat, srcPtr += 16 * REPEAT_MAX * srcStride, dstPtr += 16 * REPEAT_MAX, num_subtile_y -= REPEAT_MAX) {
...
template <typename T, unsigned blockElemSize, unsigned dstStride, unsigned srcStride>
__PTO_INSTR__ void TransposeB16(__ubuf__ T *dstPtr, __ubuf__ T *srcPtr, unsigned num_subtile_x, unsigned num_subtile_y)
{
...
template <typename T, unsigned blockElemSize, unsigned dstStride, unsigned srcStride>
__PTO_INSTR__ void TransposeB8(__ubuf__ T *dstPtr, __ubuf__ T *srcPtr, unsigned num_subtile_x, unsigned num_subtile_y)
{
    ...
    __ubuf__ T *dstPtr, __ubuf__ T *srcPtr, unsigned num_subtile_x, unsigned num_subtile_y)
{
    ...
    if constexpr (sizeof(T) == 4) {  // b32 case
        TransposeB32<T, blockElemSize, dstStride, srcStride>(dstPtr, srcPtr, num_subtile_x, num_subtile_y);
    } else if constexpr (sizeof(T) == 2) {
        TransposeB16<T, blockElemSize, dstStride, srcStride>(dstPtr, srcPtr, num_subtile_x, num_subtile_y);
    } else if constexpr (sizeof(T) == 1) {
        TransposeB8<T, blockElemSize, dstStride, srcStride>(dstPtr, srcPtr, num_subtile_x, num_subtile_y);
    } else {
...
__PTO_INSTR__ void copy_rows_with_mask(__ubuf__ T *dstPtr, __ubuf__ T *srcPtr, unsigned num_row, unsigned num_tail)
{
    ...
    if constexpr (sizeof(T) == 4) {
        ...
        if (num_tail > 1) {
            SetContMaskByDType<T>(num_tail >> 1);
            vcopy(
                (__ubuf__ uint16_t *)dstPtr, (__ubuf__ uint16_t *)srcPtr, num_row, 1, 1, dstRepeatSize, srcRepeatSize);
        }
    ...
    __ubuf__ T *dstPtr, __ubuf__ T *srcPtr, unsigned num_tail_x, unsigned num_subtile_y)
{
    ...
    for (unsigned iter = 0; iter < full_iter_num; ++iter) {
        TransposeFullSubTiles<T, blockElemSize, tmpStride, srcStride>(
    ...
    if (const uint16_t tmpSubtilesTail = num_subtile_y % tmpSubtilesMax) {
        ...
        TransposeFullSubTiles<T, blockElemSize, tmpStride, srcStride>(
            tmpPtr, (srcPtr + full_iter_num * tmpStride * srcStride), 1, tmpSubtilesTail);
        pipe_barrier(PIPE_V);
        copy_ubuf_to_ubuf(
            (dstPtr + full_iter_num * tmpStride), tmpPtr, 0, num_tail_x, tailBurst, tmpTailGap, dstTailGap);
        pipe_barrier(PIPE_V);
    }
}
    ...
    __ubuf__ T *dstPtr, __ubuf__ T *srcPtr, unsigned num_subtile_x, unsigned num_tail_y)
{
    ...
    for (unsigned iter = 0; iter < full_iter_num; ++iter) {
        ...
        pipe_barrier(PIPE_V);
        copy_rows_with_mask<T, dstStride, tmpStride>(
            (dstPtr + iter * tmpRowsMax * dstStride), tmpPtr, tmpRowsMax, num_tail_y);
        pipe_barrier(PIPE_V);
    }

    if (const unsigned tmpSubtilesTail = num_subtile_x % tmpSubtilesMax) {
        TransposeFullSubTiles<T, blockElemSize, tmpStride, srcStride>(
            tmpPtr, (srcPtr + full_iter_num * tmpSubtilesMax * blockElemSize), tmpSubtilesTail, 1);
        pipe_barrier(PIPE_V);
        copy_rows_with_mask<T, dstStride, tmpStride>(
            (dstPtr + full_iter_num * tmpRowsMax * dstStride), tmpPtr, tmpSubtilesTail * blockElemSize, num_tail_y);
        pipe_barrier(PIPE_V);
    }
}
    ...
    __ubuf__ T *dstPtr, __ubuf__ T *srcPtr, unsigned num_tail_x, unsigned num_tail_y)
{
    ...
    __ubuf__ T *tmpPtr = (__ubuf__ T *)(TMP_UB_OFFSET);  // 8KB, start from 184KB, UB:192KB=184+8KB
    TransposeFullSubTiles<T, blockElemSize, tmpStride, srcStride>(tmpPtr, srcPtr, 1, 1);
    pipe_barrier(PIPE_V);
    ...
    typename TileData::TileDType __in__ src, unsigned validRow, unsigned validCol)
{
    ...
    TransposeFullSubTiles<T, blockElemSize, dstStride, srcStride>(
        dstPtr, srcPtr, num_full_subtile_x, num_full_subtile_y);

    TransposeXTailSubtiles<T, dstStride, srcStride>(
        dstPtr + (validCol - num_tail_x) * dstStride, srcPtr + (validCol - num_tail_x), num_tail_x, num_full_subtile_y);

    TransposeYTailSubtiles<T, dstStride, srcStride>(
        dstPtr + (validRow - num_tail_y), srcPtr + (validRow - num_tail_y) * srcStride, num_full_subtile_x, num_tail_y);

    TransposeXYTailSubtile<T, blockElemSize, dstStride, srcStride>(
        dstPtr + (validCol - num_tail_x) * dstStride + (validRow - num_tail_y),
        srcPtr + (validRow - num_tail_y) * srcStride + (validCol - num_tail_x),
        num_tail_x, num_tail_y);
}
...
__PTO_INSTR__ void TTRANS_IMPL(TileDataDst &dst, TileDataSrc &src)
{
    ...
    static_assert(
        sizeof(T) == sizeof(typename TileDataDst::DType), "TTRANS: Inconsistent source and destination data types.");
