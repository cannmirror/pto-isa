...
namespace pto
{
    ...
    __tf__ PTO_INTERNAL void TMulS(typename TileData::TileDType __out__ dst,
                                ...
                                unsigned validCol) 
    {
        ...
        TBinSInstr<MulSOp<typename TileData::DType>, TileData, elementsPerRepeat, blockSizeElem, stride>(
                dstPtr, src0Ptr, src1, validRow, validCol);
    }
    template <typename TileData>
    PTO_INTERNAL void TMULS_IMPL(TileData &dst, TileData &src0, typename TileData::DType scalar)
    {
        ...
        TMulS<TileData, elementsPerRepeat, blockSizeElem, stride>(dst.data(), src0.data(), scalar, validRow, validCol);
    }
    ...
    __tf__ PTO_INTERNAL void TMulS(typename TileDataDst::TileDType __out__ dstData,
        ...
        if constexpr (std::is_same_v<TileDataDst, TileDataSrc>) {
            ...
            TBinSInstr<MulSOp<T>, T, TileDataDst, elementsPerRepeat, blockSizeElem, stride>(dst, src, scalar, validRow, validCol);
        } else {
            TBinSPlusInstr<MulSOp<T>, T, TileDataDst, TileDataSrc>(dst, src, scalar, validRow, validCol);
        }
    }
