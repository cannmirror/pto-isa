...
PTO_INTERNAL void TSelsHead(__ubuf__ T *dstPtr, __ubuf__ T *src0Ptr, __ubuf__ T *src1Ptr, unsigned validRow,
                            unsigned numRepeatPerLine, uint8_t opSelectionMode)
{
    ...
    for (uint64_t i = 0; i < validRow; i++) {
        if (numLoop) {
            for (int j = 0; j < numLoop; j++) {
                vsel(dstPtr + i * stride + j * elementsPerRepeat * REPEAT_MAX,
        ...
        if (remainAfterLoop) {
            vsel(dstPtr + i * stride + numLoop * elementsPerRepeat * REPEAT_MAX,
...
PTO_INTERNAL void TSelsTailHead(__ubuf__ T *dstPtr, __ubuf__ T *src0Ptr, __ubuf__ T *src1Ptr, unsigned numLoop,
                                uint8_t opSelectionMode)
{
    for (uint64_t i = 0; i < numLoop; i++) {
        if constexpr (strideOverFlag) {
            for (uint64_t j = 0; j < REPEAT_MAX; j++) {
                vsel(dstPtr + i * REPEAT_MAX * stride + j * stride, src0Ptr + i * REPEAT_MAX * stride + j * stride,
                     src1Ptr + i * REPEAT_MAX * stride + j * stride, 1, 1, 1, 1, 1, 1, 1, opSelectionMode);
            }
        } else {
            vsel(dstPtr + i * REPEAT_MAX * stride, src0Ptr + i * REPEAT_MAX * stride, src1Ptr + i * REPEAT_MAX * stride,
...
PTO_INTERNAL void TSelsTail(__ubuf__ T *dstPtr, __ubuf__ T *src0Ptr, __ubuf__ T *src1Ptr, unsigned validRow,
                            unsigned numRemainPerLine, uint8_t opSelectionMode)
{
    ...
    if (remainAfterLoop > 0) {
        if constexpr (strideOverFlag) {
            for (uint64_t j = 0; j < remainAfterLoop; j++) {
                vsel(dstPtr + numLoop * REPEAT_MAX * stride + j * stride,
                     src0Ptr + numLoop * REPEAT_MAX * stride + j * stride,
                     src1Ptr + numLoop * REPEAT_MAX * stride + j * stride, 1, 1, 1, 1, 1, 1, 1, opSelectionMode);
            }
        } else {
            vsel(dstPtr + numLoop * REPEAT_MAX * stride, src0Ptr + numLoop * REPEAT_MAX * stride,
                 src1Ptr + numLoop * REPEAT_MAX * stride, remainAfterLoop, 1, 1, 1, stride / blockSizeElem,
                 stride / blockSizeElem, stride / blockSizeElem, opSelectionMode);
        }
    }
    set_vector_mask(-1, -1);
}
template <typename TileData, unsigned elementsPerRepeat, unsigned blockSizeElem, unsigned stride>
__tf__ AICORE void TSelsImpl(typename TileData::TileDType __out__ dst, typename TileData::TileDType __in__ src0,
                             typename TileData::TileDType __in__ src1, uint8_t scalar, unsigned validRow,
...
PTO_INTERNAL void TSELS_IMPL(TileData &dst, TileData &src0, TileData &src1, uint8_t selectMode)
{
    ...
    TSelsImpl<TileData, elementsPerRepeat, blockSizeElem, stride>(dst.data(), src0.data(), src1.data(), selectMode,
                                                                  validRow, numRepeatPerLine, numRemainPerLine);
}
} // namespace pto
#endif