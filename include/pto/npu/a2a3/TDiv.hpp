...
__tf__ PTO_INTERNAL void TDiv(typename TileDataDst::TileDType __out__ dstData,
    ...
    if constexpr (std::is_same_v<TileDataDst, TileDataSrc0> && std::is_same_v<TileDataDst, TileDataSrc1>) {
        ...
        BinaryInstr<DivOp<T>, T, TileDataDst, elementsPerRepeat, blockSizeElem, rowStride>(dst, src0, src1, validRow, validCol);
    } else {
        BinaryPlusInstr<DivOp<T>, T, TileDataDst, TileDataSrc0, TileDataSrc1>(dst, src0, src1, validRow, validCol);
    }
}
...
PTO_INTERNAL void TDIV_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
    ...
    static_assert(std::is_same<T, typename TileDataSrc0::DType>::value ||
                  ...
                  "TDIV: The data type of dst must be consistent with of src0 and src1.");

    static_assert(std::is_same<T, half>::value || std::is_same<T, float16_t>::value ||
    ...
    if ((validRow == src0.GetValidRow() && validCol == src0.GetValidCol()) &&
        ...
        TDiv<T, TileDataDst, TileDataSrc0, TileDataSrc1>(dst.data(), src0.data(), src1.data(), validRow, validCol);
    } else {
        PTO_ASSERT(false, "TDIV: dstTile validRow/validCol must be consistent with of src0 and src1.");
    }
}
}  // namespace pto
#endif