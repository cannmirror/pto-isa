...
namespace pto {
    ...
    template <typename T> struct ColExpandMulOp {
    ...
    __tf__ AICORE void TColExpandMul(typename TileData::TileDType __out__ dst, 
        ...
        ColExpandBinaryInstr<ColExpandMulOp<T>, TileData, TileDataSrc, elementsPerRepeat, blockSizeElem, rowStride>(
    ...
    PTO_INTERNAL void TCOLEXPANDMUL_IMPL(TileData &dst, TileData &src0, TileDataSrc &src1) {
        static_assert(std::is_same<typename TileData::DType, float>::value ||
                      ...
                      "TCOLEXPANDMUL: Invalid data type.");
        static_assert(TileData::isRowMajor, "TCOLEXPANDMUL: not supported Layout type");
        ...
        TColExpandMul<TileData, TileDataSrc, elementsPerRepeat, blockSizeElem, rowStride>(dst.data(), src0.data(), src1.data(), validRow, validCol);
    }
}
#endif