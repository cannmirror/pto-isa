...
__tf__ PTO_INTERNAL OP_NAME(TSHR) OP_TYPE(element_wise) void TShr(typename TileData::TileDType __out__ dst,
    ...
    if constexpr (std::is_same<T, int8_t>::value || std::is_same<T, int16_t>::value ||
        ...
        if constexpr (dstRowStride == src0RowStride && dstRowStride == src1RowStride) {
            BinaryInstr<ShrOp<T>, TileData, elementsPerRepeat, blockSizeElem, dstRowStride>(
                dstPtr, src0Ptr, src1Ptr, validRows, validCols, version);
        } else {
            BinaryInstr<ShrOp<T>, TileData, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride, src1RowStride>(
                dstPtr, src0Ptr, src1Ptr, validRows, validCols, version);
        }
    } else {
        if constexpr (dstRowStride == src0RowStride && dstRowStride == src1RowStride) {
            BinaryInstr<ShrOp2<T>, TileData, elementsPerRepeat, blockSizeElem, dstRowStride>(
                dstPtr, src0Ptr, src1Ptr, validRows, validCols, version);
        } else {
            BinaryInstr<ShrOp2<T>, TileData, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride, src1RowStride>(
                dstPtr, src0Ptr, src1Ptr, validRows, validCols, version);
        }
    }
    return;
...
PTO_INTERNAL void TShrCheck(const TileDataDst &dst, const TileDataSrc0 &src0, const TileDataSrc1 &src1) {
    static_assert(std::is_same<T, typename TileDataSrc0::DType>::value &&
        std::is_same<T, typename TileDataSrc1::DType>::value, "Fix: TShr has invalid data type.");
    static_assert(std::is_same<T, uint8_t>::value || std::is_same<T, int8_t>::value ||
        std::is_same<T, uint16_t>::value || std::is_same<T, int16_t>::value ||
        std::is_same<T, uint32_t>::value || std::is_same<T, int32_t>::value, "Fix: TShr has invalid data type.");
