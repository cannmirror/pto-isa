...
PTO_INTERNAL void TPartAddInstr(__ubuf__ T *dstPtr, __ubuf__ T *src0Ptr, __ubuf__ T *src1Ptr, unsigned validRow,
                                unsigned validCol)
{
    __VEC_SCOPE__
    {
                                  ...
                                  unsigned dstValidRow, unsigned dstValidCol)
{
    ...
    if (condSrc0EqDst && condSrc1EqDst) { // src0 == src1 == dst
        TPartAddInstr<T, TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem, dstRowStride,
        ...
        if (src0ValidCol != 0) {
            TPartAddInstr<T, TileDataDst, TileDataSrc0, TileDataDst, elementsPerRepeat, blockSizeElem, dstRowStride,
                          src0RowStride, dstRowStride>(dstPtr, src0Ptr, dstPtr, src0ValidRow, src0ValidCol);
        }
    } else if (condSrc0RowLtDst && condSrc1EqDst) { // src0Row < dstRow
        if (src0ValidRow != 0) {
            TPartAddInstr<T, TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem, dstRowStride,
                          src0RowStride, src1RowStride>(dstPtr, src0Ptr, src1Ptr, src0ValidRow, src0ValidCol);
        }
        TPartCopyInstr<T, TileDataDst, TileDataSrc1, blockSizeElem, dstRowStride, src1RowStride>(
        ...
        if (src1ValidCol != 0) {
            TPartAddInstr<T, TileDataDst, TileDataSrc1, TileDataDst, elementsPerRepeat, blockSizeElem, dstRowStride,
