...
namespace pto {
    ...
    __tf__ AICORE void TRowExpandSub(typename TileDataDst::TileDType __out__ dst, 
        ...
        if(src0eqdst){
            RowExpandBinaryInstr<RowExpandSubOp<T>, TileDataDst, TileDataSrc1, elementsPerRepeat, blockSizeElem, rowStride>(
                                dstPtr, src0Ptr, src1Ptr, validRow, validCol);
        } else {
            RowExpandBinaryInstr<RowExpandSubOp2<T>, TileDataDst, TileDataSrc1, elementsPerRepeat, blockSizeElem, rowStride>(
                                dstPtr, src0Ptr, src1Ptr, validRow, validCol);
        }
    }
    ...
    PTO_INTERNAL void TROWEXPANDSUB_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
        ...
        if constexpr (src0eqdst) {
            ...
            TRowExpandSub<TileDataDst, TileDataSrc1, elementsPerRepeat, blockSizeElem, rowStride, src0eqdst>(dst.data(), src0.data(), src1.data(), validRow, validCol);
        } else  {
            ...
            TRowExpandSub<TileDataDst, TileDataSrc0, elementsPerRepeat, blockSizeElem, rowStride, src0eqdst>(dst.data(), src1.data(), src0.data(), validRow, validCol);
        }
    }
}
#endif