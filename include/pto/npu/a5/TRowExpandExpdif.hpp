...
namespace pto {
    ...
    __tf__ AICORE OP_NAME(TROWEXPANDEXPDIF) OP_TYPE(broadcast) void TRowExpandExpdif(typename TileDataDst::TileDType __out__ dst, 
        ...
        if(src0eqdst){
            RowExpandBinaryInstr<RowExpandExpdifOp<T>, TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
                                dstPtr, src0Ptr, src1Ptr, validRow, validCol);
        } else {
            RowExpandBinaryInstr<RowExpandExpdifOp2<T>, TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
                                dstPtr, src0Ptr, src1Ptr, validRow, validCol);
        }
    }
    ...
    PTO_INTERNAL void TROWEXPANDEXPDIF_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
        ...
        if (src0eqdst) {
            ...
            TRowExpandExpdif<TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(dst.data(), src0.data(), src1.data(), validRow, validCol, src0eqdst);
        } else  {
            unsigned src0ValidCol = src0.GetValidCol();
            ...
            TRowExpandExpdif<TileDataDst, TileDataSrc1, TileDataSrc0, elementsPerRepeat, blockSizeElem>(dst.data(), src1.data(), src0.data(), validRow, validCol, src0eqdst);
        }
    }
}
#endif