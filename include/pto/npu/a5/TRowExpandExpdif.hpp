...
namespace pto {
    ...
    template <typename T> struct RowExpandExpdifOp2 {
        PTO_INTERNAL static void RowExpandBinaryInstr(RegTensor<T> &reg_dst, RegTensor<T> &reg_src0, RegTensor<T> &reg_src1, MaskReg &preg)
        {
            if constexpr (std::is_same_v<T, float>) {
    ...
    __tf__ AICORE OP_NAME(TROWEXPANDEXPDIF) OP_TYPE(broadcast) void TRowExpandExpdif(typename TileDataDst::TileDType __out__ dst, 
        ...
        if(src0eqdst){
            ...
            RowExpandBinaryInstr<RowExpandExpdifOp2<T>, TileDataDst, TileDataSrc1, elementsPerRepeat, blockSizeElem, rowStride>(
                                dstPtr, src0Ptr, src1Ptr, validRow, validCol);
        }
    }
    ...
    PTO_INTERNAL void TROWEXPANDEXPDIF_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
        ...
        static_assert(
            ...
            "Fix: TROWEXPANDEXPDIF Invalid data type.");
        ...
        static_assert(TileDataDst::isRowMajor && (src0eqdst || src1eqdst), "Fix: TROWEXPANDEXPDIF Invalid tile shape.");
        ...
        if constexpr (src0eqdst) {
            ...
            PTO_ASSERT(((TileDataSrc1::isRowMajor && src1ValidCol == 32 / sizeof(T)) ||
                        ...
                        src1.GetValidRow() == validRow, "TROWEXPANDEXPDIF: invalid src1 shape.");
            TRowExpandExpdif<TileDataDst, TileDataSrc1, elementsPerRepeat, blockSizeElem, rowStride, src0eqdst>(dst.data(), src0.data(), src1.data(), validRow, validCol);
        } else  {
            ...
            PTO_ASSERT(((TileDataSrc0::isRowMajor && src0ValidCol == 32 / sizeof(T)) ||
                        ...
                        src0.GetValidRow() == validRow, "TROWEXPANDEXPDIF: invalid src0 shape.");
            TRowExpandExpdif<TileDataDst, TileDataSrc0, elementsPerRepeat, blockSizeElem, rowStride, src0eqdst>(dst.data(), src1.data(), src0.data(), validRow, validCol);
        }
    }
}
#endif