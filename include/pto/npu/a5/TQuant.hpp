...
PTO_INTERNAL void ExtractB8ExponentAndScaling(__ubuf__ float* maxPtr,
    ...
    for (uint16_t i = 0; i < (uint16_t) exp_max_loop_count; ++i){
        ...
        if constexpr (unroll)
            vsts((vector_s32 &)vb32_scaling, (vector_s32 &)vb32_scaling, ((__ubuf__ int32_t *)scalingPtr), 2*i*elementsPerRepeat, INTLV_B32, preg_b32);
        else 
...
PTO_INTERNAL void CalcQuantizedFP8Values_Unroll2(__ubuf__ float* srcPtr,
                                        __ubuf__ float* scalingPtr,
                                        __ubuf__ uint8_t* dstPtr,
                                        unsigned vl_count,
                                        unsigned elementsPerRepeat,
...
__tf__ PTO_INTERNAL void TQuant(typename TileDataSrc::TileDType __in__  src,
    ...
    __VEC_SCOPE__ {
        ...
        if constexpr (unroll_condition) {
            CalcQuantizedFP8Values_Unroll2(srcPtr, scalingPtr, (__ubuf__ uint8_t*)dstPtr, vl_count, 
