...
namespace pto {
    ...
    template <typename T> struct ColExpandDivOp {
    ...
    __tf__ AICORE void TColExpandDiv(typename TileData::TileDType __out__ dst, 
    ...
    PTO_INTERNAL void TCOLEXPANDDIV_IMPL(TileData &dst, TileData &src0, TileDataSrc &src1) {
        static_assert(std::is_same<typename TileData::DType, float>::value ||
                      ...
                      "TCOLEXPANDDIV: Invalid data type.");
        static_assert(TileData::isRowMajor, "TCOLEXPANDDIV: not supported Layout type");
        ...
        TColExpandDiv<TileData, TileDataSrc, elementsPerRepeat, blockSizeElem, rowStride>(dst.data(), src0.data(), src1.data(), validRow, validCol);
    }
}
#endif