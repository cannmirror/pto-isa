...
namespace pto {
    ...
    PTO_INTERNAL void TMUL_IMPL(TileData &dst, TileData &src0, TileData &src1) {
        using T = typename TileData::DType;
        TMulCheck<T, TileData>();
        ...
        TMul<TileData, elementsPerRepeat, blockSizeElem, rowStride>
    ...
    PTO_INTERNAL void TMUL_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1)
    {
        static_assert(std::is_same_v<TileDataDst, TileDataSrc0> && std::is_same_v<TileDataDst, TileDataSrc1>,
                      "TMUL: Input tileshape must be consistent with the out tileshape.");
        using T = typename TileDataDst::DType;
        TMulCheck<T, TileDataDst>();
        constexpr unsigned blockSizeElem = BLOCK_BYTE_SIZE / sizeof(T); 
        constexpr unsigned elementsPerRepeat = REPEAT_BYTE / sizeof(T); 
        ...
        TMul<TileDataDst, elementsPerRepeat, blockSizeElem, rowStride>
            (dst.data(), src0.data(), src1.data(), validRow, validCol);
    }
}
#endif
