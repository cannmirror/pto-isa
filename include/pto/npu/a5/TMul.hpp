...
namespace pto {
    ...
    AICORE void TMUL_IMPL(TileData &dst, TileData &src0, TileData &src1) {
        ...
        TMul<TileData, elementsPerRepeat, blockSizeElem, rowStride>(dst.data(), src0.data(), src1.data(), validRow, validCol);
    }
    ...
    PTO_INTERNAL void TMUL_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1)
    {
        static_assert(std::is_same_v<TileDataDst, TileDataSrc0> && std::is_same_v<TileDataDst, TileDataSrc1>,
                      "Fix: TMUL Input tileshape must be consistent with the out tileshape.");
