...
namespace pto {
    ...
    template <typename T> struct RowExpandMinOp {
    ...
    __tf__ AICORE OP_NAME(TROWEXPANDMIN) OP_TYPE(broadcast) void TRowExpandMin(typename TileDataDst::TileDType __out__ dst, 
        ...
        RowExpandBinaryInstr<RowExpandMinOp<T>, TileDataDst, TileDataSrc1, elementsPerRepeat, blockSizeElem, rowStride>(
    ...
    PTO_INTERNAL void TROWEXPANDMIN_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
        ...
        static_assert(TileDataDst::isRowMajor && (src0eqdst || src1eqdst), "Fix: TROWEXPANDMIN Invalid tile shape.");
        ...
        if constexpr (src0eqdst) {
            ...
            PTO_ASSERT(((TileDataSrc1::isRowMajor && src1ValidCol == 32 / sizeof(T)) ||
                        ...
                        src1.GetValidRow() == validRow, "TROWEXPANDMIN: invalid src1 shape.");
            TRowExpandMin<TileDataDst, TileDataSrc1, elementsPerRepeat, blockSizeElem, rowStride>(dst.data(), src0.data(), src1.data(), validRow, validCol);
        } else  {
            ...
            PTO_ASSERT(((TileDataSrc0::isRowMajor && src0ValidCol == 32 / sizeof(T)) ||
                        ...
                        src0.GetValidRow() == validRow, "TROWEXPANDMIN: invalid src0 shape.");
            TRowExpandMin<TileDataDst, TileDataSrc0, elementsPerRepeat, blockSizeElem, rowStride>(dst.data(), src1.data(), src0.data(), validRow, validCol);
        }
    }
}
#endif