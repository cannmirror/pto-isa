...
namespace pto {
    ...
    PTO_INTERNAL void TROWEXPANDMUL_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
        ...
        if (src0eqdst) {
            ...
            TRowExpandMul<TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(dst.data(), src0.data(), src1.data(), validRow, validCol);
        } else  {
            unsigned src0ValidCol = src0.GetValidCol();
            ...
            TRowExpandMul<TileDataDst, TileDataSrc1, TileDataSrc0, elementsPerRepeat, blockSizeElem>(dst.data(), src1.data(), src0.data(), validRow, validCol);
        }
    }
}
#endif