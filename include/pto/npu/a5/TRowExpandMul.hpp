...
namespace pto {
    ...
    __tf__ AICORE void TRowExpandMul(typename TileData::TileDType __out__ dst, 
        ...
        RowExpandBinaryInstr<RowExpandMulOp<T>, TileData, TileDataSrc, elementsPerRepeat, blockSizeElem, rowStride>(
                                dstPtr, src0Ptr, src1Ptr, validRow, validCol);
    }

    template <typename TileData, typename TileDataSrc>
    PTO_INTERNAL void TROWEXPANDMUL_IMPL(TileData &dst, TileData &src0, TileDataSrc &src1) {
        static_assert(std::is_same<typename TileData::DType, float>::value ||
                      std::is_same<typename TileData::DType, half>::value,
                      "TROWEXPANDMUL: Invalid data type.");
        static_assert(TileData::isRowMajor, "TROWEXPANDMUL: not supported Layout type");
        ...
        TRowExpandMul<TileData, TileDataSrc, elementsPerRepeat, blockSizeElem, rowStride>(dst.data(), src0.data(), src1.data(), validRow, validCol);
    }
}
#endif