...
namespace pto {
    ...
    template <typename T> struct RowExpandSubOp {
    ...
    template <typename T> struct RowExpandSubOp2 {
        PTO_INTERNAL static void RowExpandBinaryInstr(RegTensor<T> &reg_dst, RegTensor<T> &reg_src0, RegTensor<T> &reg_src1, MaskReg &preg)
        {
    ...
    __tf__ AICORE OP_NAME(TROWEXPANDSUB) OP_TYPE(broadcast) void TRowExpandSub(typename TileDataDst::TileDType __out__ dst, 
        ...
        if(src0eqdst){
            RowExpandBinaryInstr<RowExpandSubOp<T>, TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
                                dstPtr, src0Ptr, src1Ptr, validRow, validCol);
        } else {
            RowExpandBinaryInstr<RowExpandSubOp2<T>, TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
                                dstPtr, src0Ptr, src1Ptr, validRow, validCol);
        }
    }
    ...
    PTO_INTERNAL void TROWEXPANDSUB_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
        ...
        if (src0eqdst) {
            ...
            TRowExpandSub<TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(dst.data(), src0.data(), src1.data(), validRow, validCol, src0eqdst);
        } else  {
            unsigned src0ValidCol = src0.GetValidCol();
            ...
            TRowExpandSub<TileDataDst, TileDataSrc1, TileDataSrc0, elementsPerRepeat, blockSizeElem>(dst.data(), src1.data(), src0.data(), validRow, validCol, src0eqdst);
        }
    }
}
#endif