...
namespace pto {
    ...
    PTO_INTERNAL void TROWEXPANDMIN_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
        ...
        static_assert(std::is_same_v<T, half> || std::is_same_v<T, float>, "Fix: TROWEXPANDMIN Invalid data type.");
        ...
        if (src0eqdst) {
            ...
            TRowExpandMin<TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(dst.data(), src0.data(), src1.data(), validRow, validCol);
        } else  {
            unsigned src0ValidCol = src0.GetValidCol();
            ...
            TRowExpandMin<TileDataDst, TileDataSrc1, TileDataSrc0, elementsPerRepeat, blockSizeElem>(dst.data(), src1.data(), src0.data(), validRow, validCol);
        }
    }
}
#endif